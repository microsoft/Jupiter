...
class MCTSTree:
    ...
    def eval_final_answer(self, node):
        ...
        if self.ground_truth:
            # 训练模式：有 ground_truth，进行正确性检查
            correct_sub_problem = check_correct(self.ground_truth, final_answer)
            reward = POSITIVE_REWARD * correct_sub_problem / max(self.ground_truth.count("@"), 1) \
                if correct_sub_problem > 0 else NEGETIVE_REWARD
            node.update_recursive(reward, self.root)
        elif self.is_test_mode:
            # 测试模式：收集候选答案并给予中性奖励
            self.collect_candidate_answer(node, raw_content)
    ...
    def collect_candidate_answer(self, node, raw_content: str):
        """收集候选答案信息"""
        final_answer = extract_final_answer(raw_content)
        if final_answer is None:
            return  # 不保存无效答案
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='MCTS Inference for AIME (from Parquet)')
    parser.add_argument('--test_mode', action='store_true', help='Enable test mode (evaluation without ground truth)')
    parser.add_argument('--parquet_path', type=str, default='../dataset/aime2025/aime2025.parquet', help='Path to AIME parquet file')
    ...
    # 读取 parquet 数据
    df = pd.read_parquet(args.parquet_path)[:100]
    if args.limit:
